...
// ── XP / LEVEL ──────────────────────────────────────
#define MAXLVL 15
const char* TITLES[]={"","Freshly Dead","Shambler   ","Groaner    ","Crawler    ",
  "Walker     ","Runner     ","Infected   ","Feral      ","Ravager    ",
  "Brain Eater","Necromancer","Undead Lord","Zombie God ","APOCALYPSE ","EXTINCTION "};
uint32_t xp=0, xpNext=100; uint8_t lvl=1; bool lvlPending=false;
uint32_t xpForLvl(uint8_t l){return (uint32_t)l*100;}
...
void writeSessionStats(){
  ...
  if(f){
    ...
    snprintf(line,64,"level:          %d  (%s)",lvl,TITLES[lvl<MAXLVL+1?lvl:MAXLVL]); f.println(line);
...
Ach ACHS[NACH]={
  ...
  {"l15", "EXTINCTION",       "Reach max level 15",        "You win",        false},
...
const Bdr BDRS[]={
  ...
  {"SKULL","b25",3},{"GLITCH","s5",4},{"CROWN","l15",5},
};
const Acc ACCS[]={
  {"NONE",NULL,"",""},    {"HAT","n10"," ^^^^ ",""},
...
const Bdg BDGS[]={
  ...
  {"SCANNER","[-]","n50"},{"MAXED","^X^","l15"},{"VETERAN","|V|","sec5"},
};
const Flr FLRS[]={
...
void drawTitle(int16_t x,int16_t y,uint8_t l){
  ...
  sprintf(b,"%s%s%s",(eFlr&&cunlock(f.lock))?f.pre:"",TITLES[l<MAXLVL+1?l:MAXLVL],(eFlr&&cunlock(f.lock))?f.suf:"");
  txt(x,y,b,DIM,BG,1);
}
...
void showStats(){
  ...
  sprintf(b,"lv%d  %s",lvl,TITLES[lvl<MAXLVL+1?lvl:MAXLVL]); row("level:",b);
...
void showWardrobe(){
  ...
  auto draw=[&](){
    ...
    for(int i=0;i<4;i++){
      int x=i*80;
      if(i==tab){fillRect(x,0,80,16,FG);txt(x+4,5,tn[i],BG,FG,1);}
      else{fillRect(x,0,80,16,BG);fillRect(x,0,1,16,DIM);txt(x+4,5,tn[i],DIM,BG,1);}
    }
    fillRect(0,16,SW,1,FG);
    if(tab==0){
      for(int i=0;i<NBD;i++){
        int y=20+i*33; bool u=cunlock(BDRS[i].lock); bool eq=(eBdr==i);
        fillRect(0,y,SW,32,BG);fillRect(0,y,SW,1,DIM);
        txt(4,y+8,u?BDRS[i].name:"?LOCKED",u?FG:DIM,BG,2);
        if(eq) txt(200,y+10,"<EQUIPPED>",FG,BG,1); else if(u) txt(240,y+10,"tap",DIM,BG,1);
      }
    } else if(tab==1){
      for(int i=0;i<NAC;i++){
        int y=20+i*27; bool u=cunlock(ACCS[i].lock); bool eq=(eAcc==i);
        fillRect(0,y,SW,26,BG);fillRect(0,y,SW,1,DIM);
        const char*pv=strlen(ACCS[i].top)>0?ACCS[i].top:ACCS[i].side; if(!strlen(pv))pv="--";
        txt(4,y+6,u?pv:"???",u?FG:DIM,BG,1);
        txt(52,y+4,ACCS[i].name,u?FG:DIM,BG,2);
        if(eq) txt(220,y+8,"<ON>",FG,BG,1); else if(u) txt(250,y+8,"tap",DIM,BG,1);
      }
    } else if(tab==2){
      txt(4,20,"EQUIPPED:",DIM,BG,1);
      for(int i=0;i<MXBDG;i++){
        int bx=80+i*26;
      ...
      for(int i=0;i<NBG;i++){
        ...
        bool u=cunlock(BDGS[i].lock); bool eq=isBdgOn(i);
        fillRect(x,y,58,42,BG);
        uint16_t brd=eq?FG:DIM;
        fillRect(x,y,58,1,brd);fillRect(x,y+41,58,1,brd);fillRect(x,y,1,42,brd);fillRect(x+57,y,1,42,brd);
        txt(x+14,y+4,u?BDGS[i].icon:"???",u?FG:DIM,BG,2);
        int nl=strlen(BDGS[i].name)*6; txt(x+(58-nl)/2,y+26,BDGS[i].name,DIM,BG,1);
      }
    } else {
      txt(4,22,"TITLE FLAIR",FG,BG,2); fillRect(0,42,SW,1,DIM);
      for(int i=0;i<NFL;i++){
        int y=46+i*28; bool u=cunlock(FLRS[i].lock); bool eq=(eFlr==i);
        char pv[32]; sprintf(pv,"%s%s%s",FLRS[i].pre,"Brain Eater",FLRS[i].suf);
        fillRect(0,y,SW,27,BG);fillRect(0,y,SW,1,DIM);
        txt(4,y+6,u?pv:"???LOCKED",u?FG:DIM,BG,1);
        if(eq) txt(250,y+6,"<ON>",FG,BG,1); else if(u) txt(260,y+6,"tap",DIM,BG,1);
      }
    }
    fillRect(0,231,SW,1,FG); txt(4,234,"short=next tab  hold=exit",DIM,BG,1);
  };
  draw();
  while(true){
    yield(); delay(30);
    ...
    uint32_t t=millis(); while(digitalRead(PIN_BTN)==LOW)delay(10);
    if(millis()-t>=800) break;      // long = exit
    tab=(tab+1)%4; draw();          // short = next tab
  }
}
